...
func putInstance(c echo.Context) error {
	...
	if _, err := database.Exec("INSERT INTO instances VALUES (NULL, ?, ?)", addr, c.QueryParam("bastion") != ""); err != nil {
		return err
	}
...
func deleteInstance(c echo.Context) error {
	...
	result, err := database.Exec("DELETE FROM instances WHERE addr = ?", addr)
	if err != nil {
		return err
	}
